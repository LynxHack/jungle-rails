...
RSpec.describe User, type: :model do
  ...
  describe '.authenticate_with_credentials' do
    it 'should log the user in if the credentials are correct' do
      @user = User.new(first_name: "John", last_name: "Smith", email: "test@gmail.com", password: "ABCDEF", password_confirmation: "ABCDEF")
      @user.save!
      expect(User.authenticate_with_credentials("test@gmail.com", "ABCDEF")).to be_present
    end

    it 'should not log the user in if the email is wrong' do
      @user = User.new(first_name: 'John', last_name: 'Smith', email: 'test@gmail.com', password:'ABCDEF', password_confirmation:'ABCDEF')
      @user.save!
      expect(User.authenticate_with_credentials('tester@gmail.com','ABDCEF')).not_to be_present;
    end

    it 'should not log the user in if the password is wrong' do
      @user = User.new(first_name: 'John', last_name: 'Smith', email: 'test@gmail.com', password:'ABCDEF', password_confirmation:'ABCDEF')
      @user.save!
      expect(User.authenticate_with_credentials('test@gmail.com','ABDCsdfsEF')).not_to be_present;
    end
